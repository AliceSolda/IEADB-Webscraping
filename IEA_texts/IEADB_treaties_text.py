...
#to run this code, you need to create a folder where you are going to store
#the treaties' text.

list_text_id = [3059, 8501, 3100, 5006] #add all agreements id in there


i = 0
inc = 0
n = len(list_text_id) #with n the total number of agreements
...
while i <= n-1:  
    ...
    mytext = myDiv.findAll('p') #creates a list of all the "p"s   
    ...
    for line in mytext: 
        new_line = str(line)
        cleanr = re.compile('<.*?>') 
        cleantext = re.sub(cleanr, '', new_line) #clean the text from the html tag
        cleantext = cleantext + "\n"
